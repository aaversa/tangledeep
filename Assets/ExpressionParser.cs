...
public class ExpressionParser
{
    ...
    //Impact Optimization: we use this on every time
    private System.Func<double[], double> m_rndFunc;

    public ExpressionParser()
    {
        var rnd = new System.Random();

        //Impact Optimizations: Removing unused consts and functions
    ...
    IValue Parse(string aExpression)
    {
        ...
        if (reps >= 200) {
        	UnityEngine.Debug.Log("Broke engine parser while loop");
        }
        ...
        //
        //  check RND first because it's so often used
        //  check for $ before marching through all consts
        //  removed almost all the math from m_Funcs since we hardly use them
        //
        //  "If I'm using sine and cosine on damage numbers I have failed."
        //
        //                                              -- Andrew "Maths" Aversa RIP 21XX
        //
        //

        if (aExpression.StartsWith("rnd"))
        {
            var inner = aExpression.Substring(3);   //length of "rnd"
            var param = Parse(inner);
            var multiParams = param as MultiParameterList;
            IValue[] parameters = multiParams != null ? multiParams.Parameters : new IValue[] { param };
            return new CustomFunction("rnd", m_rndFunc, parameters);
        }

        //Impact defined consts use $ at the start because Andrew is reasonable
        ...
        */
        ...
        throw new ParseException("Reached unexpected end within the parsing tree, expression was: " + aExpression + "  ");
    }
